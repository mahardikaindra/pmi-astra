...
function PageComponent() {
  ...
  const fetchJalur = async () => {
    try {
      const colRef = collection(db, "artifacts/Ij8HEOktiALS0zjKB3ay/jalur");
      const snap = await getDocs(colRef);

      const jalurData: Jalur[] = snap.docs.map((docSnap) => {
        const data = docSnap.data() as Jalur;
        return { id: docSnap.id, ...data };
      });

      setJalurs(jalurData);
    } catch (e) {
      console.error("Error fetching jalur:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirm = async () => {
    if (!selectedJalur?.id) return;

    try {
      const docRef = doc(
        db,
        `artifacts/Ij8HEOktiALS0zjKB3ay/jalur/${selectedJalur.id}`,
      );
      await deleteDoc(docRef);

      setJalurs((prev) => prev.filter((j) => j.id !== selectedJalur.id));
      setIsOpen(false);
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-100 p-4 pt-45">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-xl font-bold mb-4 text-gray-800">List Jalur</h1>
          {canCRUD && (
            <Link
              href="/management/jalur/add"
            ...
            .map((jalur) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{jalur.lokasi}</p>

                  <p className="text-xs text-gray-500">
                    Panjang: {jalur.panjang} km
                  </p>
                  <p className="text-xs text-gray-500">
                    Status: {jalur.status}
                  </p>
                ...
                <div className="flex flex-col gap-2">
                  <Link
                    href={`/management/jalur/${jalur.id}`}
                    title="View"
                  ...
                  {canCRUD && (
                    <Link
                      href={`/management/jalur/${jalur.id}/edit`}
                      title="Edit"
      ...
      {isOpen && (
        <CustomPopup
          title="Konfirmasi"
          message={`Apakah Anda yakin ingin menghapus jalur "${
            selectedJalur?.nama || ""
          }"?`}
          onClose={() => {
            setIsOpen(false);
...
const JalurPage = dynamic(() => Promise.resolve(PageComponent), {
  ssr: false,
});
