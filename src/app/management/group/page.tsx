...
function PageComponent() {
  ...
  const fetchGroup = async () => {
    try {
      const colRef = collection(db, "artifacts/Ij8HEOktiALS0zjKB3ay/group");
      const snap = await getDocs(colRef);

      const jalurData: Group[] = snap.docs.map((docSnap) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const docRef = doc(
        db,
        `artifacts/Ij8HEOktiALS0zjKB3ay/group/${selectedGroup.id}`,
      );
      await deleteDoc(docRef);

      setGroups((prev) => prev.filter((j) => j.id !== selectedGroup.id));
      setIsOpen(false);
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-100 p-4 pt-45">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-xl font-bold mb-4 text-gray-800">List Group</h1>
          {canCRUD && (
            <Link
              href="/management/group/add"
            ...
            .map((group) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{group.deskripsi}</p>
                ...
                <div className="flex flex-col gap-2">
                  <Link
                    href={`/management/group/${group.id}`}
                    title="View"
                  ...
                  {canCRUD && (
                    <Link
                      href={`/management/group/${group.id}/edit`}
                      title="Edit"
      ...
      {isOpen && (
        <CustomPopup
          title="Konfirmasi"
          message={`Apakah Anda yakin ingin menghapus group "${
            selectedGroup?.nama || ""
          }"?`}
          onClose={() => {
            setIsOpen(false);
...
const GroupPage = dynamic(() => Promise.resolve(PageComponent), {
  ssr: false,
});
